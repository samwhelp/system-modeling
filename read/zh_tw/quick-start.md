...
然後我會「File Manager」切換到資料夾，

按下「滑鼠右鍵」，出現「功能選單」，針對「檔案」或「資料夾」做一些操作，

例如，我會在某個資料夾，按下右鍵，然後使用「atom」或「sublime-text」來開啟「專案」。
...
就可以在視窗的「Frame」，使用「滑鼠左鍵按住拖曳」，不限於在「視窗標題列」。
...
就可以在視窗的「Frame」，使用「滑鼠左鍵按住拖曳」，不限於在「視窗邊框」。
